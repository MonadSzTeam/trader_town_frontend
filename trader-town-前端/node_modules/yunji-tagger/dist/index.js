...
var TagInjector = class {
  ...
  processVueNode(node, context) {
    try {
      ...
      const projectRoot = process.cwd();
      const relativePath = path2.relative(projectRoot, filename);
...
var TagInjector2 = class {
  ...
  processReactNode(node, state) {
    ...
    const relativePath = path3.relative(root, filename);
    const fileName = path3.basename(filename);
    safeAddAttr(
...
var createReactInjectorPlugin = (options = {}) => {
  ...
  const filter = createFilter2(options.include || /\.(jsx|tsx)$/, options.exclude || /node_modules/);
...
export {
  DEFAULT_ATTRIBUTES,
  react_injector_plugin_default as ReactInjectorVitePlugin,
  vue_injector_plugin_default as VueInjectorVitePlugin,
  html_injector_plugin_default as createHTMLInjectorPlugin
};
